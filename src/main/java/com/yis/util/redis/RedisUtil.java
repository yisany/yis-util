...
public class RedisUtil {
    ...
    public static boolean initInstance(String host, int port, String password) {
        return initRedis(host, port, password, DB);
    }

    public static boolean initInstance(String host, int port, String password, int db) {
        return initRedis(host, port, password, db);
    }
    ...
    public static RedisUtil getRedisUtil() {
        if (handler == null) {
            synchronized (RedisUtil.class) {
                if (handler == null) {
                    logger.error("RedisUtil is not inited, please init");
                    throw new BizException("init RedisUtil first, use RedisUtil.initInstance method");
                }
            }
        }
        return handler;
    ...
    private static boolean initRedis(String host, int port, String password, int db) {
        if (handler == null) {
            synchronized (RedisUtil.class) {
                if (handler == null) {
                    handler = new RedisUtil();
                    handler.init(host, port, password, db);
                    return true;
                }
            }
        }
    ...
    private void init(String host, int port, String password, int db) {
        if (pool == null) {
            JedisPoolConfig configs = new JedisPoolConfig();
            configs.setMaxIdle(MAX_IDLE);
            configs.setMaxTotal(MAX_TOTAL);
            configs.setMaxWaitMillis((long)MAX_WAIT_MILLIS);
            if (StringUtils.isNotBlank(password)) {
                pool = new JedisPool(configs, host, port, TIME_OUT, password);
            } else {
                pool = new JedisPool(configs, host, port, TIME_OUT);
            }
        }
    }
